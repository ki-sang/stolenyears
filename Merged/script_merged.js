...
d3.csv("names_children.csv").then(function(data2) {
  ...
  // Global variable to keep track of the total number of stars on the screen
  let starsOnScreen = [];
  var starsOnScreenint = 0;
 ...
 */
  ...
  function createStar(value) {
    const star = svg.append("text")
      .attr("class", "star") // star.className = 'star';
      .attr("x", Math.random() * window.innerWidth) // star.style.left = Math.random() * window.innerWidth + 'px';
      .attr("y", Math.random() * window.innerHeight) // star.style.top = Math.random() * window.innerHeight + 'px';
      .attr("fill", "white")
      .attr("font-size", (Math.random() * 5+5) + "px") // star.style.fontSize = (Math.random() * 5 + 5) + 'px';
      .text(JSON.stringify(value).replace('{"names_en":"','').replace('"}', '')); // star.textContent = JSON.stringify(value).replace('{"names_en":"','').replace('"}', '');

    star.transition()
      .duration(Math.random() * 4000 + 5000) // star.style.animationDuration = (Math.random() * 4 + 5) + 's';
      .ease(d3.easeCubic) // star.style.animationTimingFunction = 'ease-in-out';
      .attr("opacity", 1)
      .transition()
      .duration(3000) // Fade out duration // star.style.transition = 'opacity 3s';
      .delay (40000) // Fade out delay from start // }, 40000); // Fade-out delay since beginning
      .attr("opacity", 0)
      .on("end", () => { // star.addEventListener('animationend', () => {
        star.remove(); // Remove star after animation ends
        });

  // Limit number of stars on screen
  let starsOnScreen = [];
  var starsOnScreenint = 0;
  starsOnScreenint += 1;
  if (starsOnScreenint > 1000) {
    const oldestStar = starsOnScreen.shift();
    oldestStar.remove();
  }
  ...
  setInterval(() => {
    ...
    createStar(removeValue);
  }, 1500); // Time interval between each new star
  }
